...
void display() 
{
	...
	if (drawsilhouette)
	{
		...
		for (vector<myHalfedge *>::iterator it = m->halfedges.begin(); it != m->halfedges.end(); it++)
		{
			...
			myVector3D V(camera_eye.X - v1->point->X,
				camera_eye.Y - v1->point->Y,
				camera_eye.Z - v1->point->Z);

			double scalaire1 = *(e->adjacent_face->normal) * V;

			double scalaire2 = *(e->twin->adjacent_face->normal) * V;


			if ( scalaire1*scalaire2 <= 0 /*ADD THE CONDITION TO CHECK IF THE HALFEDGE DEFINED BY (V1, V2) IS A SILHOUETTE EDGE*/ )
			{
